...
$sql = "SELECT c.libelle, e.jour, DATE_FORMAT(e.heure_debut, '%H:%i') as debut, DATE_FORMAT(e.heure_fin, '%H:%i') as fin, e.salle 
...
<head>
    <meta charset="UTF-8">
    <title>SmartCampus - Emploi du temps</title>
    <style>
        ...
        .badge { background: #e0e7ff; color: #4338ca; padding: 5px 10px; border-radius: 999px; font-size: 0.85rem; font-weight: 600; }
    </style>
</head>
<body>
    <div class="sidebar">
        <h2>SMARTCAMPUS</h2>
        <a href="etudiant_dashboard.php">Mes Notes</a>
        <a href="emploi_du_temps.php" class="active">Emploi du temps</a>
    ...
    <div class="content">
        ...
        <div class="card">
            <table>
                <thead>
                    <tr><th>Jour</th><th>Horaires</th><th>Matière</th><th>Salle</th></tr>
                </thead>
                <tbody>
                    <?php foreach($edt as $cours): ?>
                    <tr>
                        <td><strong><?= htmlspecialchars($cours['jour']); ?></strong></td>
                        <td><?= htmlspecialchars($cours['debut'] . " - " . $cours['fin']); ?></td>
                        <td><?= htmlspecialchars($cours['libelle']); ?></td>
                        <td><span class="badge"><?= htmlspecialchars($cours['salle']); ?></span></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
